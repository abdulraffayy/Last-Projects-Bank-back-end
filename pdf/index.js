...
// Fetch line items from your API
async function fetchLineItems() {
  try {
    const response = await axios.get('http://localhost:3003/api/invoicelineitems'); // Replace with your API URL
    return response.data; // Returning the data to be used in the PDF
  } catch (error) {
...
async function fetchInvoiceData() {
  try {
    const response = await axios.get('http://localhost:3003/api/invoices'); // Replace with your API endpoint
    return response.data; // Returning the invoice data
  } catch (error) {
...
async function generatePDF() {
  const lineItems = await fetchLineItems(); // Fetch line items from the API
  const invoiceData = await fetchInvoiceData(); // Fetch invoice data from the API
  ...
  lineItems.forEach((item) => {
    doc.text(item.serviceId?.name || 'N/A', 50, tableY)
       .text(item.serviceId?.fee?.toString() || '0', 220, tableY)
       .text(item.hoursDays?.toString() || '0', 320, tableY)
       .text(item.amount?.toString() || '0', 450, tableY);
  ...
  invoiceData.forEach((invoice) => {
    doc
      .text(`Name: ${invoice.clientId?.name || 'N/A'}`, y + 15)
      .text(`Parent/Guardian: ${invoice.clientId?.guardian || 'N/A'}`, 50, y + 30)
      .text(`Address: ${invoice.clientId?.address || 'N/A'}`, 50, y + 45);
  ...
  // --- Client Image (Right Side) --- (Optional: Only if image exists)
  try {
    doc.image('../assets/bsesPhotos.png', 350, y + 15, { width: 150 });
  } catch (error) {
    console.error('Client image not found, skipping image section.');
  }
